...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.system.RAG_EMBEDDING_MODEL': {
    title: 'RAG 语义向量模型',
    summary: '为投资知识库生成文档与查询向量，用于召回没有直接关键词重合但语义相关的内容。',
    usage: '填写 LiteLLM 支持的 embedding 模型，例如 openai/text-embedding-3-small 或 ollama/bge-m3；留空时继续使用 FTS5 与词法检索。',
    valueNotes: [
      '生成模型与 embedding 模型用途不同，不要直接填写普通聊天模型。',
      '切换模型后，旧向量不会参与新模型检索，需要在知识库页面更新语义索引。',
    ],
    impact: ['启用后，新投喂文档会自动生成向量，检索会融合语义、BM25 与词法结果。'],
    notes: ['使用远程模型时，文档 chunk 和检索问题会发送到所配置的 embedding 服务。'],
  },
  'settings.system.RAG_EMBEDDING_BASE_URL': {
    title: 'RAG 向量服务地址',
    summary: '指定 OpenAI-compatible embedding 服务地址；留空时由 LiteLLM 按模型 provider 使用默认地址。',
    usage: '通常填写包含 /v1 的 API 根地址，并确保该服务实现 embeddings 接口。',
    impact: ['影响知识库向量生成和语义查询请求的目标服务。'],
    notes: ['修改后需要重启后端；切换服务或模型后建议重建语义索引。'],
  },
  'settings.system.RAG_EMBEDDING_API_KEY': {
    title: 'RAG 向量服务密钥',
    summary: '为知识库 embedding 请求单独配置访问凭据。',
    usage: '仅在向量服务需要鉴权时填写；留空时 LiteLLM 会尝试读取对应 provider 的标准环境变量。',
    impact: ['影响向量生成和语义查询是否能通过服务端鉴权。'],
    notes: ['该字段按敏感信息处理，不会以明文回显。'],
  },
  'settings.system.RAG_EMBEDDING_DIMENSIONS': {
    title: 'RAG 向量维度',
    summary: '覆盖支持可变维度的 embedding 模型输出维度。',
    usage: '通常保持 0 使用模型默认维度；只有服务明确支持 dimensions 参数时才填写正整数。',
    impact: ['影响向量存储体积和相似度计算；维度变化后必须重建语义索引。'],
    notes: ['不支持可变维度的模型应保持 0。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.system.RAG_EMBEDDING_MODEL': {
    ...
    summary: 'Generates document and query vectors so the investment knowledge base can recall semantically related content without exact keyword overlap.',
    usage: 'Enter a LiteLLM-supported embedding model such as openai/text-embedding-3-small or ollama/bge-m3. Leave empty to keep FTS5 and lexical retrieval only.',
    valueNotes: [
      'Chat/generation models and embedding models serve different purposes; do not use a regular chat model here.',
      'After changing models, rebuild the semantic index because vectors from the old model are excluded.',
    ],
    impact: ['New documents are embedded automatically and searches fuse semantic, BM25, and lexical rankings.'],
    notes: ['With a remote model, document chunks and search queries are sent to the configured embedding service.'],
  },
  'settings.system.RAG_EMBEDDING_BASE_URL': {
    title: 'RAG Embedding Base URL',
    summary: 'Sets an OpenAI-compatible embedding endpoint. When empty, LiteLLM uses the provider default.',
    usage: 'Usually provide the API root including /v1 and ensure the service implements the embeddings endpoint.',
    impact: ['Controls the destination for knowledge-base embedding and semantic query requests.'],
    notes: ['Restart the backend after changes and rebuild the semantic index after switching services or models.'],
  },
  'settings.system.RAG_EMBEDDING_API_KEY': {
    title: 'RAG Embedding API Key',
    summary: 'Configures a dedicated credential for knowledge-base embedding requests.',
    usage: 'Set only when the endpoint requires authentication. When empty, LiteLLM can use the provider standard environment variable.',
    impact: ['Controls whether embedding and semantic-query requests pass provider authentication.'],
    notes: ['This field is treated as sensitive and is not echoed in plain text.'],
  },
  'settings.system.RAG_EMBEDDING_DIMENSIONS': {
    title: 'RAG Embedding Dimensions',
    summary: 'Overrides the output size for embedding models that support variable dimensions.',
    usage: 'Keep 0 to use the model default. Set a positive integer only when the provider supports the dimensions parameter.',
    impact: ['Changes vector storage size and similarity calculations; rebuild the semantic index after changing it.'],
    notes: ['Keep 0 for models that do not support variable dimensions.'],
  },
  'settings.system.MAX_WORKERS': {
    title: 'Max Workers',
